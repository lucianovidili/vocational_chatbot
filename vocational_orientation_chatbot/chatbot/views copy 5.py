...
def chatbot_response(request):
    if request.method == "POST":
        try:
            # Leemos el mensaje del usuario, pero no lo usamos para el contenido
            data = json.loads(request.body)
            user_message = data.get("message", "")

            # Prompt fijo que le pedimos al bot
            format_prompt = """
You are an expert career counselor who provides kind and helpful advice.

Please provide a numbered list of 3 important career advice points.
Each item should be on its own line, numbered as 1), 2), 3), etc.
"""
